...
/**
 * Generate or retrieve a random salt for key derivation
 */
async function getOrCreateSalt() {
  const SALT_KEY = '_pgSalt';
  const stored = await chrome.storage.local.get(SALT_KEY);
  
  if (stored[SALT_KEY]) {
    return stored[SALT_KEY];
  }
  
  // Generate new random salt
  const crypto = getCrypto();
  const saltArray = crypto.getRandomValues(new Uint8Array(32));
  const salt = bufferToBase64(saltArray);
...
async function getCryptoKey() {
  ...
  // Use runtime ID + random salt for better security
